...
export default function VideoAdPlayer({ params }) {
  ...
  const getmovie = async () =>{
    ...
    let r = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/get`, {method: "POST", headers: {"Content-Type": "application/json"},
  ...
  const handleSkip = async () => {
    ...
    const { movie } = await params
    const movieName = movie.replaceAll("-", " ")
    let r = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/movie`, {method: "POST", headers: {"Content-Type": "application/json"},
            body: JSON.stringify({ movieName }) })
        let  res = await r.json();
        setmovieurl(res)
  ...
  const handledownloadskip = async () => {
    ...
    const { movie } = await params
    const movieName = movie.replaceAll("-", " ")
    let r = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/download`, {method: "POST", headers: {"Content-Type": "application/json"},
            body: JSON.stringify({ movieName }) })
        let  res = await r.json();
        setdownloadlink(res)
  ...
  return (
    <div className='lg:container lg:mx-auto bg-[#111111] lg:min-h-[93vh] min-h-[100vh] text-white lg:px-10 '>
      {moviesData ? (<div className="movies flex flex-col items-center">
        <div className="movieStream flex justify-center">
        {showAd ? (
        ...
        <div className='relative   cursor-pointer' onClick={adplay}>
          ...
          <div className="play absolute lg:top-[12%] lg:left-[47%] top-[30%] left-[45%] sm:top-[25%] sm:left-[42%] z-10 cursor-pointer" onClick={adplay}>
